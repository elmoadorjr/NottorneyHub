...
def batch_upload_data(client, endpoint: str, data_key: str, items: List[Dict], 
                      extra_body: Dict = None, batch_size: int = API_BATCH_SIZE) -> Dict[str, Any]:
    """
    Helper to upload data in batches.
    
    Args:
        client: The API client instance (must have .post method)
    ...
    for i in range(0, total, chunk_size):
        ...
        try:
            result = client.post(endpoint, json_body=body)
